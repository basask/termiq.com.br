interface LogoMarkProps {
  size?: number
  /** Render only the dominant wave stroke (used in the mobile header) */
  minimal?: boolean
}

export default function LogoMark({ size = 26, minimal = false }: LogoMarkProps) {
  ...
  return (
    <svg
      ...
      width={size}
      height={h}
      aria-label={minimal ? undefined : 'TermIQ'}
      aria-hidden={minimal || undefined}
    >
      {!minimal && (
        <>
