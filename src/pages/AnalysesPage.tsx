...
const HEADERS = ['Name', 'Cycle', 'Cycle start', 'Created', '']
  ...
  return (
    <div className="p-4 md:p-7 flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-tq-fg-1">Analyses</h1>
          <p className="font-mono text-[12px] text-tq-fg-3 mt-1">
            {analyses.length} analys{analyses.length !== 1 ? 'es' : 'is'} · linked to device cycles
          </p>
        </div>
        <Button asChild>
      ...
      <Card>
        <CardHeader className="p-4">
          ...
          <CardDescription>Thermal analyses linked to fetched device cycles</CardDescription>
        </CardHeader>
        <CardContent className="p-0 overflow-x-auto">
          {analyses.length === 0 ? (
            <div className="flex items-center gap-3 px-4 py-8 text-[13px] text-tq-fg-3">
              ...
              by linking it to a fetched cycle.
            </div>
          ) : (
      ...
      <div className="flex items-center gap-3 px-4 py-3 rounded-lg border border-tq-border bg-tq-bg-soft text-[13px] text-tq-fg-3">
        ...
        Analysis comparison, export, and sharing are coming soon.
      </div>
    </div>
  )
}
