/**
 * Thermograph USB Protocol Driver (JS port of poc_device_client.py)
 *
 * Transport : USB HID, vendor usage page 0xFFA0, 32-byte interrupt endpoints
 * Endpoints : EP_OUT = 0x02 (host → device), EP_IN = 0x82 (device → host, number = 2)
 */
...
export interface IThermographDriver {
  open(): Promise<void>
  close(): Promise<void>
  getInfo(): Promise<DeviceInfo>
  listRecords(): Promise<RecordEntry[]>
  fetchRecord(recordIdx: number, numChannels?: number): Promise<FetchedRecord>
}
...
export class ThermographDriver implements IThermographDriver {
  ...
  async open(): Promise<void> {
    ...
    // HID SET_IDLE — tells device to stop sending idle reports
    ...
    // HID SET_REPORT — device-specific init payload observed in usbmon captures
    const initPayload = new Uint8Array(32)
    initPayload.set([0xff, 0xc7, 0x83, 0xcc, 0x10, 0x00, 0x00, 0x00])
    await this.device.controlTransferOut(
      { requestType: 'class', recipient: 'interface', request: 0x09, value: 0x0200, index: 0x0000 },
      initPayload,
    )
  ...
  private async recv(): Promise<Uint8Array> {
    const result = await this.device.transferIn(EP_IN, PKT_LEN)
    if (!result.data) throw new Error('No data received from device')
  ...
  /** Reads ping + date + time + channels + interval + datapoints + status in one call. */
  ...
  /** Enumerates all stored records by walking CMD_LIST until metadata is empty. */
  ...
  /**
   * Downloads a complete record as a multi-packet stream.
   *
   * Packet format:
   *   byte[0]   = N (valid payload bytes in bytes[1..N])
   *   First packet strips a 5-byte sub-header [55 52 03 idx ??]
   *   Temperature: 2-byte LE uint16 / 10.0 = °C
   *   Channels interleaved: ch1_t0, ch2_t0, … chN_t0, ch1_t1 …
   *   Stream ends at first 0xAA byte
   */
